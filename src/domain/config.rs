...
mod tests {
    ...
    use super::{
        ...
        FullLiveRouterAllowanceMode, LoadedDomainConfig, TokenBQuoteSizing,
    };
    ...
    fn committed_esp_full_live_policy_has_permanent_bounds() {
        let loaded = load(ESP_PRODUCTION_CONFIG.as_bytes()).unwrap();
        let pair = &loaded.snapshot().pairs[0];
        ...
        for (path, replacement) in [
            ...
            let mut value: Value = serde_json::from_str(ESP_PRODUCTION_CONFIG).unwrap();
            *value.pointer_mut(path).unwrap() = replacement;
            assert!(load(&serde_json::to_vec(&value).unwrap()).is_err());
        }

        let mut missing_policy: Value = serde_json::from_str(ESP_PRODUCTION_CONFIG).unwrap();
