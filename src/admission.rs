...
pub struct AdmissionEconomics {
    ...
    /// Candidate cost plus maximum gas, retained as a cost-model diagnostic.
    pub gas_burdened_cost_token_a: U256,
    /// Expected profit after the maximum gas budget. This is telemetry only;
    /// Rails-compatible opportunity admission is decided by the gross spread.
    ...
    /// Legacy telemetry alias for `expected_profit_after_gas_token_a`.
...
mod tests {
    ...
    #[test]
    fn dex_first_admission_keeps_after_gas_profit_as_diagnostic() {
        let quote = top_of_book(Decimal::from(10), Decimal::ONE);
    ...
    #[test]
    fn after_gas_profit_is_diagnostic_only() {
        let mut request = inputs(ArbitrageDirection::BuyTokenBOnDexSellOnCex);
        request.expected_proceeds_token_a = U256::from(10_001_000_u64);
        let negative_after_gas = evaluate_admission(&book(), request).unwrap().unwrap();

        assert!(negative_after_gas.opportunity_threshold_met);
        assert_eq!(
            negative_after_gas.expected_profit_token_a,
            U256::from(1_000_u64)
        );
        assert_eq!(negative_after_gas.bounded_profit_token_a, U256::ZERO);

        request.expected_proceeds_token_a = U256::from(10_040_000_u64);
        let positive_after_gas = evaluate_admission(&book(), request).unwrap().unwrap();
        assert!(positive_after_gas.opportunity_threshold_met);
        assert_eq!(
            positive_after_gas.expected_profit_after_gas_token_a,
            U256::from(2_500_u64)
        );

        request.opportunity_threshold_met = false;
        let without_gross_proof = evaluate_admission(&book(), request).unwrap().unwrap();
        assert!(!without_gross_proof.opportunity_threshold_met);
        assert_eq!(
            without_gross_proof.expected_profit_after_gas_token_a,
            U256::from(2_500_u64)
        );
    }
