...
| Binance ESP address verification | exact Arbitrum Satoshi deposit test | Approved / deploying | Binance's authenticated read-only Address Verification API returned exactly one matching `USDC` / `ARBITRUM` record for the production wallet with status `PENDING`, owner flag `1`, and Satoshi verification method `1`. The operator-approved artifact permits one journaled transfer of exactly `0.9987 USDC` from that wallet to Binance deposit address `0x64d62673799a8dc69825ff1cc0d624b1065dab39`; bridge is forbidden and receipt/final-verification evidence remains pending. |
