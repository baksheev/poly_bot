...
| Binance rejected ESP withdrawal reconciliation | exact prior Travel Rule incident | Failed closed, corrected | The exact v2 row is `trId=67181540`, `travelRuleStatus=4`, with omitted `withdrawalStatus` and no transaction hash. Recovery refetches the exact `trId`, rejects identity changes or any broadcast/completed evidence, proves capital history empty, validates the durable `401.2 ESP` subaccount-to-master transfer, and requires master Spot to remain exactly `401.2 ESP` free/zero locked. Rails originally used the standard capital endpoint; commit `6520658` globally switched it to local-entity. Live read-only evidence now also proves the exact Arbitrum wallet has `whiteStatus=true`; the M9 policy pins USDC to `travel_rule` and ESP to `standard`, with the API mode journaled before submission and no automatic retry after an ambiguous request. |
| Binance M9 USDC standard probe | exact durable 3 USDC operation | Failed closed, recovery approved | Operation `rebalance-272-c36484318ac04bca` has internal transfer `395795295053` in `SUCCESS`, unchanged wallet balance `24.0013 USDC`, zero capital and Travel Rule withdrawal rows, no transaction hash, master free balance `2201.44553984 USDC`, ownership `VERIFIED`, and an Arbitrum withdrawal whitelist record with `whiteStatus=true`. Because the account questionnaire is `AE` and prior USDC Travel Rule withdrawal succeeded, the same operation resumes through `travel_rule`; it may not create another internal transfer. |
