...
| Binance rejected ESP withdrawal reconciliation | exact prior Travel Rule incident | Failed closed, corrected | The first post-verification init proved the exact address record was `VERIFIED`, then stopped before any withdrawal because Binance's Travel Rule v2 history did not stably index the synchronous HTTP `400` / `-4024` rejection. Recovery now permits the versioned incident to close with no history row only when both withdrawal histories are empty, the exact rejected ESP debit and durable successful master transfer match, and no amount/address/network candidate is pending or broadcast. Any ambiguity still fails closed; the retry remains restricted to the same Travel Rule API. |
