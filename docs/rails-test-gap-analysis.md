...
| Market and pool state | Partial | Persistent Binance `bookTicker`; REST-bootstrapped `depth@100ms` with strict `U..u` continuity, separate depth-health telemetry, tiered exact/recent/top-only adaptive inputs, and full-depth bounded recovery quotes; sequence-aware World Chain logs/heads; local V3/V4 state and quotes | Fault-inject Binance/Alchemy disconnect, gap, reorg, and stale-state recovery |
...
| P0-3 | Shared inventory and reservations | Partial | One in-memory owner hydrates free Binance, wallet-token, and wallet-native balances. Baseline and adaptive admission atomically reserve `exact_execution_envelope_v1` with no Rails multiplier; live rebalance reserves its exact source; unknown outcomes remain held; balanced work releases only after both venues advance. Live order-rate claims still need the same owner. |
