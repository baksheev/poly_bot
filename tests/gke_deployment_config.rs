...
#[test]
fn gke_manifest_is_the_full_live_v10_adaptive_owner() {
    assert!(
        RELEASE_PLATFORM
            .contains("DOMAIN_CONFIG_PATH: config/strategies/usdc-wld-world-chain.v10.json")
    );
...
fn gke_workflow_verifies_the_runtime_startup_mode() {
    ...
    assert!(DEPLOY_WORKFLOW.contains("usdc-wld-world-chain.v10.json"));
