...
- The production GKE Pod must use the reviewed v7 adaptive-shadow domain
  artifact with both arbitrage and rebalancing in `full_live`. Adaptive shadow
  must never change the executed baseline quantity. The deployment workflow
  must verify those startup fields before it reports success.
